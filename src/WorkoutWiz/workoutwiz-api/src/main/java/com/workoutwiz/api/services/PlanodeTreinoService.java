...
public class PlanodeTreinoSerive { 
...
public static void registerPlanodeTreino(PlanodeTreinoModel planodeTreino, RegisterPlanodeTreinoCallback callback) {
    DatabaseManager.execute((success, connection) -> {
        if (success) {
            String sql = "INSERT INTO PlanodeTreino (ClienteID, Data, Frequencia, Modalidade, GanhoDeMassaMuscular, ControleDePeso, AumentoDaResistencia, ProgressoesDeCarga, ProgressoesDeDificuldade, TreinadorID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            try (PreparedStatement stmt = connection.prepareStatement(sql)) {
                stmt.setInt(1, planodeTreino.getClienteID());
                stmt.setDate(2, planodeTreino.getData());
...
public static void queryPlanodeTreino(QueryPlanodeTreinoCallback callback) {
    DatabaseManager.execute((success, connection) -> {
        if (success) {
            String sql = "SELECT ClienteID, Data, Frequencia, Modalidade, GanhoDeMassaMuscular, ControleDePeso, AumentoDaResistencia, ProgressoesDeCarga, ProgressoesDeDificuldade, TreinadorID FROM PlanodeTreino";
            try (PreparedStatement stmt = connection.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    PlanodeTreinoModel planodeTreinoModel = new PlanodeTreinoModel(clienteID, data, frequencia, modalidade, ganhoDeMassaMuscular, controleDePeso, aumentoDaResistencia, progressoesDeCarga, progressoesDeDificuldade, treinadorID);
                    planodeTreinoList.add(planodeTreinoModel);
                }

                callback.onResult(planodeTreinoList, true); // Callback com sucesso
            } catch (SQLException e) {
                System.out.println("Erro ao consultar Plano de Treino: " + e.getMessage());
